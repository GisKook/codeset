...
extern Datum qhcardinfo_monitor(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(qhcardinfo_monitor);

Datum
qhcardinfo_monitor(PG_FUNCTION_ARGS)
{
	...
	if (!CALLED_AS_TRIGGER(fcinfo))
		elog(ERROR, "qhcardinfo_monitor: not called by trigger manager");
